...
func traverse(ctx context.Context, node *cdp.Node) error {
	...
	fmt.Printf("[%d] %d %s %s %d %+v\n", node.NodeID, node.NodeType, node.NodeName, node.NodeValue, node.ChildNodeCount, node.Children)
	nodeParams, err := dom.DescribeNode().WithNodeID(node.NodeID).WithDepth(1).WithPierce(true).Do(ctx)
	if err != nil {
		return errors.New(fmt.Sprintf("error while describing node id %d: %s", node.NodeID, err.Error()))
	}
	...
	for _, n := range nodeParams.Children {
		...
		err = traverse(ctx, n)
		if err != nil {
			return errors.New(fmt.Sprintf("error while traversing child node %d from node %d: %s", n.NodeID, node.NodeID, err.Error()))
		}
	}
	return nil
}
